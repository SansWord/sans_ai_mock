...
- [x] ~~**`start_folder/README.md` still names `feature.spec.md` / `feature.plan.md`.** Claude-Code/Superpowers convention leaking into candidate-facing instructions. Either soften to "an example workflow" or strip.~~ (Already addressed — verified 2026-04-29 that neither filename appears in `start_folder/README.md` or `todo_feature.md`. The current candidate-facing text uses generic phrasing: "draft a spec/plan, write tests first, implement, verify." Marking complete; the cleanup landed in an earlier release without being crossed off.)
