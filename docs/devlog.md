...
|---|---|
| [v0.9.8](#v098--reviewer-driven-readme-fixes--inline-feedback-excerpt-2026-04-29-0053) | Address findings from a paired senior-eng + cross-file consistency review: add an inline feedback excerpt under "Real run example" (50-word verdict block from the example mock — closes the "is the rubric real or vague?" gap that was top-2 senior-eng blocker); add direct link to `feedback.md` (one less click); fix "on Opus 4.7" cost attribution to acknowledge the Sonnet 4.6 mid-round swap; nuance Pro-tier guidance — fine on Sonnet 4.6, tight on Opus 4.7; sync Pro phrasing across README and example folder; add a forward-pointer parenthetical at `INTERVIEWER.md:14` so the no-edit rule isn't read as absolute before the NOTE / feedback exceptions are introduced two lines later; cross off the `feature.spec.md / feature.plan.md` roadmap item (verified neither name appears in `start_folder/`) |
...
## v0.9.8 — reviewer-driven README fixes + inline feedback excerpt (2026-04-29 00:53)

**Review:** not yet

**What was built:**
- **Inline feedback excerpt under "Real run example"** — 50-word block quoting the verdict paragraph from `examples/2026-04-28-todo-list/feedback.md`, condensed from the 87-word original by dropping meta-commentary and preserving the specific calibration terms ("Strong Pass-grade in isolation," "Below Bar," "didn't ship F3") + the actionable closing line ("treating the spec as a contract rather than a starting point"). Renders as ~4 lines on GitHub at default width. Senior-eng reviewer's #1 ask: *"even 4-5 lines from the example mock pasted directly in the README under 'Real run example.' A quoted snippet would close the deal."*
- **Direct link to `feedback.md`** in the "Real run example" intro sentence — one click less than the previous "open the folder, then click again." Senior-eng reviewer's #2 ask.
- **Fixed "on Opus 4.7" cost attribution** in both README and example folder. The example run was *mostly* Opus 4.7 with 26 Sonnet 4.6 messages mid-round (per `feedback.md:9`); the dollar figure now reads *"~$26 for the full experience (mostly Opus 4.7, with some Sonnet 4.6 mid-round)."*
- **Nuanced Pro-tier guidance.** Previous version said Pro is "tight" without distinguishing model. New wording: *"fine on Sonnet 4.6 (~5x cheaper per token); tight on Opus 4.7."* Plus a practical "expect to wait out a window mid-round" line for Pro+Opus users. Mirrored in both README and example folder.
- **`INTERVIEWER.md:14` forward-pointer.** The no-edit rule used to read as absolute before the NOTE / feedback exceptions appeared two lines later — a reader stopping at L14 got the wrong picture. Added a parenthetical: *"(Two narrow exceptions described immediately below: the NOTE scratch file in this repo, and the optional feedback file in the candidate's workspace at the end of Phase 3.)"*
- **Synced Pro phrasing** between README and `examples/2026-04-28-todo-list/README.md`. Both now say "before the round finishes" rather than "mid-mock" (more concrete).
- **Crossed off `roadmap.md` `start_folder/README.md feature.spec.md / feature.plan.md` item.** Verified by grep — neither filename appears in `start_folder/README.md` or `todo_feature.md`. The cleanup landed in an earlier release without being marked off; restoring honesty in the backlog.

**Key technical learnings:**
- `[insight]` Pro tier nuance matters: a 60-min mock on Sonnet 4.6 costs roughly $4 in API equivalents (~5x cheaper than Opus). That's well within Pro's 5h-window budget. The "Pro is undersized" framing was overconfidently model-agnostic — the real answer is "depends on which model you pair with." Worth saying explicitly so Pro users don't bounce on first read.
- `[note]` 50-word verdict excerpt > 87-word original for inline use. The dropped sentences (meta-commentary about "real interviewer's read" + "good news:" preamble) were valuable in the source feedback document but overflow a README inline. The compression rule: keep the specific calibration evidence + the actionable punchline; drop the framing scaffolding.

**Process learnings:**
- `[insight]` Paired-reviewer dispatch (one senior-eng-persona + one cross-file-consistency) catches different classes of issue. The senior-eng caught "feedback signal is one click away" — a UX/trust issue. The consistency reviewer caught "Opus 4.7 attribution wrong" + "INTERVIEWER.md L14 reads absolute" — internal-coherence issues. Neither would catch the other's findings reliably. For doc polish before promotion, both lenses are worth the parallel cost.
- `[note]` Marking already-completed roadmap items off as you discover them (rather than during the original change) is fine but creates a small "did this fix actually land?" verification step. Cheaper than the alternative (silent drift), but a habit of crossing-off-as-you-ship would prevent the drift in the first place.
