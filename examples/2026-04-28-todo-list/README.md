...
- **Claude Code subscription vs raw API:** the dollar figures above are API-equivalent. On a Claude Code Pro or Max plan, usage is charged against your plan's quota rather than billed directly. Rough fit:
  - **Pro ($20/mo)** — fine on Sonnet 4.6 (~5x cheaper per token); tight on Opus 4.7. The example run was mostly Opus and would likely exhaust the 5h rolling window before the round finishes. If you're on Pro and want Opus, expect to wait out a window mid-round.
